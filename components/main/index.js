...
import { DNFT_ABI, DNFT_ADDRESS } from "../../constants/contract/DLink";
...
export default function MainPage({ walletState, walletDispatch }) {
  ...
  const handleMint = async () => {
    ...
    const dnftContract = new ethers.Contract(DNFT_ADDRESS, DNFT_ABI, signer);

    let txn = await dnftContract.safeMint(userAddress, fileUrl, interval);
    txn = await txn.wait();
    const tokenCounter = await dnftContract.tokenCounter();
    const deployedAt = `https://testnets.opensea.io/assets/mumbai/0x1D943a3074D5F1721191EE6ace4F2fb1A2A4fA66/${tokenCounter}`;
                ...
                <div className="py-1 px-1">
                  ...
                  >
                    ...
                    <div className="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                      <label
                        className="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2"
                        htmlFor="trait-submit"
                      >
                        &nbsp;
                      </label>
                      <input
                        className="appearance-none block w-full   text-gray-700 border bg-blue-100 border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:border-gray-500"
                        id="trait-submit"
                        type="submit"
                        value="Add"
                      />
              ...
              <div>
                <div className="grid grid-cols-2">
                  <div>
                    ...
                    {interval ? <div>{interval} Seconds</div> : <div></div>}
                  </div>
                  <div>
                    <div>
                      {attributes &&
  ...
  function handleAttributes(e) {
    ...
    setAttributes([
      ...attributes,
      {
        trait_type: e.target[0].value,
        value: Number(e.target[1].value),
        max_value: Number(e.target[2].value),
      },
    ]);
